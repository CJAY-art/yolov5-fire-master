...
def bb_overlab(b1, bm):
    ...
    for b2 in bm:
        ...
        area1 = w1 * h1
        area2 = w2 * h2
        if overlap_area / (area1 + area2 - overlap_area)>0:
            return True
    return False
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if dataset.mode == 'image':  # 'video' or 'stream',add motion detection
                            annotator.box_label(xyxy, label, color=colors(c, True))
                        else:
                            xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # normalized xywh
                            if pm and bb_overlab(xywh, pm):
                                annotator.box_label(xyxy, label, color=colors(c, True))
                        if save_crop:
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='data/test/video', help='file/dir/URL/glob, 0 for webcam')# 'data/test/video'
    parser.add_argument('--data', type=str, default='data/fire.yaml', help='(optional) dataset.yaml path')
